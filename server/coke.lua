...
RegisterServerEvent('DjonStNix-DrugProcessing:pickedUpCocaLeaf', function()
	local src = source
...
RegisterServerEvent('DjonStNix-DrugProcessing:processCocaLeaf', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('DjonStNix-DrugProcessing:processCocaPowder', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('DjonStNix-DrugProcessing:processCocaBrick', function()
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	...
	if Player.Functions.RemoveItem('coke_small_brick', Config.CokeProcessing.SmallBrick) then
		...
		TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_enough_small_bricks", {value = Config.CokeProcessing.SmallBrick}), "error")
	end
end)

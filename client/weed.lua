...
local function RollJoint()
	...
	if lib.progressBar({
		duration = 15000,
		label = Lang:t("progressbar.rolling_joint"),
		useWhileDead = false,
		canCancel = true,
		disable = {
		...
		TriggerServerEvent('DjonStNix-DrugProcessing:rollJoint')
...
local function BagSkunk()
	...
	if lib.progressBar({
		duration = 15000,
		label = Lang:t("progressbar.bagging_skunk"),
		useWhileDead = false,
		canCancel = true,
		disable = {
			car = true, move = true, combat = true, mouse = false
		...
		TriggerServerEvent('DjonStNix-DrugProcessing:bagskunk')
		local timeLeft = Config.Delays.WeedProcessing / 1000
		while timeLeft > 0 do
			Wait(1000)
			timeLeft -= 1
...
local function ProcessWeed()
	...
	if lib.progressBar({
		...
		TriggerServerEvent('DjonStNix-DrugProcessing:processCannabis')
		local timeLeft = Config.Delays.WeedProcessing / 1000
		while timeLeft > 0 do
			Wait(1000)
			timeLeft -= 1
...
RegisterNetEvent("DjonStNix-DrugProcessing:processWeed",function()
...
RegisterNetEvent('DjonStNix-DrugProcessing:EnterWWarehouse', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('DjonStNix-DrugProcessing:ExitWWarehouse', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
...
RegisterNetEvent("DjonStNix-DrugProcessing:pickWeed", function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			if lib.progressBar({
				duration = 10000,
				label = Lang:t("progressbar.collecting"),
				useWhileDead = false,
				canCancel = true,
				disable = {
					car = true, move = true, combat = true, mouse = false
				...
				spawnedWeeds -= 1
				TriggerServerEvent('DjonStNix-DrugProcessing:pickedUpCannabis')
				isPickingUp = false
			else
				ClearPedTasks(PlayerPedId())
				isPickingUp = false
...
RegisterNetEvent('DjonStNix-DrugProcessing:client:rollJoint', function()
    QBCore.Functions.TriggerCallback('DjonStNix-DrugProcessing:validate_items', function(result)
		if result.ret then
			RollJoint()
		else
...
RegisterNetEvent('DjonStNix-DrugProcessing:client:bagskunk', function()
    QBCore.Functions.TriggerCallback('DjonStNix-DrugProcessing:validate_items', function(result)
		if result.ret then
			BagSkunk()
		else

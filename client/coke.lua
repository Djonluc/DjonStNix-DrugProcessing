...
local function ProcessCoke()
	...
	if lib.progressBar({
		...
		TriggerServerEvent('DjonStNix-DrugProcessing:processCocaLeaf')
...
local function CutCokePowder()
	...
	if lib.progressBar({
		...
		TriggerServerEvent('DjonStNix-DrugProcessing:processCocaPowder')
...
local function ProcessBricks()
	...
	if lib.progressBar({
		...
		TriggerServerEvent('DjonStNix-DrugProcessing:processCocaBrick')
...
RegisterNetEvent('DjonStNix-DrugProcessing:ProcessCocaFarm', function()
...
RegisterNetEvent('DjonStNix-DrugProcessing:ProcessCocaPowder', function()
	local coords = GetEntityCoords(PlayerPedId())
	local amount = 10
	local amount2 = 5
...
RegisterNetEvent('DjonStNix-DrugProcessing:ProcessBricks', function()
...
RegisterNetEvent('DjonStNix-DrugProcessing:EnterCWarehouse', function()
	local ped = PlayerPedId()
...
RegisterNetEvent('DjonStNix-DrugProcessing:ExitCWarehouse', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
...
RegisterNetEvent('DjonStNix-DrugProcessing:pickCocaLeaves', function()
	local playerPed = PlayerPedId()
	...
	if nearbyObject and IsPedOnFoot(playerPed) then
		if not isPickingUp then
			...
			if lib.progressBar({
				...
				TriggerServerEvent('DjonStNix-DrugProcessing:pickedUpCocaLeaf')
				isPickingUp = false
			else
				ClearPedTasks(playerPed)
				isPickingUp = false
